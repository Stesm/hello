...
class DB
{
    ...
    {
        $conf = Core::getConfig('DB')['connect'];
    ...
    public function execMulti($sql)
    {
        ...
        if (mysqli_multi_query($this->link, $sql)) {
            while (mysqli_more_results($this->link) && mysqli_next_result($this->link))
                if (mysqli_error($this->link) != '' && Core::getConfig('ENV') == 'debug')
                    throw new \Exception(mysqli_error($this->link));
        } elseif (Core::getConfig('ENV') == 'debug')
            throw new \Exception(mysqli_error($this->link));
    }
    ...
    /**
     * @param $raw_sql_string
     * @return DBRaw
     */
    public static function raw($raw_sql_string)
    {
        return new DBRaw($raw_sql_string);
    }
}
